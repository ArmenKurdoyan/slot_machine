...
SlotMachine.prototype.drawLine = function (row) {
  ...
  switch (row) {
    case "top":
      ...
      ctx.lineTo(
        this.canvas.width - middleOfImageWidth + 27,
        middleOfRowHeight - 16
      );
      break;
    case "middle":
      ...
      ctx.lineTo(
        this.canvas.width - middleOfImageWidth + 27,
        middleOfRowHeight + rowHeight + 12
      );
      break;
    case "bottom":
      ctx.moveTo(
        middleOfImageWidth - 27,
        middleOfRowHeight + rowHeight * 2 + 16
      );
      ctx.lineTo(
        this.canvas.width - middleOfImageWidth + 27,
        middleOfRowHeight + rowHeight * 2 + 16
...
SlotMachine.prototype.checkResults = function () {
  ...
  SLOTMACHINE.input_payTable.value =
    parseInt(SLOTMACHINE.input_payTable.value) + this.payTable;

  this.balance += this.payTable;
  if (this.balance > 5000) {
    this.balance = 5000;
    alert(
      "Congratulation you just have more Money than the maximum Machine Value of 5000! Take your change money in the money box."
    );
  }
 ...
 */
SlotMachine.prototype.updateState = function () {
  switch (this.currentState) {
    case 1:
      SLOTMACHINE.button_play.setAttribute("disabled", "disabled");
      SLOTMACHINE.input_balance.setAttribute("disabled", "disabled");
